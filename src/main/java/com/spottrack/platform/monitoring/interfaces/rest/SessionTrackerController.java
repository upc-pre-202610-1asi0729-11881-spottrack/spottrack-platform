...
public class SessionTrackerController {
    ...
    public ResponseEntity<?> getActiveSessionTrackers() {
        var result = sessionTrackerQueryService.handle(new GetActiveSessionTrackersQuery());
        return switch (result) {
            case Result.Success<List<SessionTracker>, ApplicationError> s ->
                    ResponseEntity.ok(s.value().stream()
                            .map(SessionTrackerResourceFromEntity::toResourceFromEntity)
                            .toList());
            case Result.Failure<List<SessionTracker>, ApplicationError> f ->
                    ResponseEntity.badRequest().body(f.error());
        };
    }
