package com.spottrack.platform.payment.application.internal.commandservices;

import com.spottrack.platform.payment.application.commandservices.PaymentCommandService;
...
public class PaymentCommandServiceImpl implements PaymentCommandService {
    ...
    @Override
    public Result<Payment, ApplicationError> handle(RecordPaymentCommand command) {
        try {
            var payment = new Payment(command);
            var saved = paymentRepository.save(payment);

            // TODO: initiate Stripe payment session here using saved.getPaymentId()
            // The Stripe webhook will call ConfirmPaymentCommand or FailPaymentCommand
